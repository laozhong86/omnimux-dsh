...
const TEMPLATES = [
  {
    ...
    author_handle: 'ecolchipro5',
    author_name: 'btfcxwfomse',
  ...
  },
  {
    ...
    author_handle: 'futurecompanion',
    author_name: 'Future Companion',
  ...
  },
  {
    ...
    author_handle: 'rubdyzq',
    author_name: 'Ruby Trends',
  ...
  },
  {
    ...
    author_handle: '10sorlabs',
    author_name: '10sor Labs',
  ...
  },
  {
    ...
    author_handle: 'gametech',
    author_name: 'Game Tech Daily',
  ...
  },
  {
    ...
    author_handle: 'skincare_master',
    author_name: 'Skincare Master',
...
async function seed() {
  ...
  for (let i = 0; i < TEMPLATES.length; i++) {
    ...
    const seedItem = {
      ...
      title: tpl.title,
      content: tpl.content,
      source_url: `https://www.tiktok.com/@${tpl.author_handle}/video/${videoId}`,
      cover_key: tpl.cover_key,
      hot_score: tpl.hot_score,
      tags: tpl.tags,
      analysis: {
        ...tpl.analysis,
        creator: {
          name: tpl.author_name,
          handle: tpl.author_handle,
        },
        tiktok_video_id: videoId,
        embed_player_url: `https://www.tiktok.com/player/v1/${videoId}`,
      },
    }
    ...
    try {
      ...
      if (createResp.status === 201 || createResp.status === 200) {
        ...
        console.error(`[${i + 1}/10] 录入失败 status ${createResp.status}:`, createJson || text)
      }
    } catch (err) {
  ...
  for (const item of verifyList) {
    ...
    console.log(`- [1] Hook亮点: ${an.hook_highlight?.slice(0, 35)}...`)
    console.log(`- [2] 转化目标: ${an.target_goal?.slice(0, 35)}...`)
    console.log(`- [3] 叙事脚本: ${an.narrative_strategy?.slice(0, 35)}...`)
    console.log(`- [4] 视觉节奏: ${an.visual_breakdown?.slice(0, 35)}...`)
    console.log(`- [5] 复刻策略: ${an.replication_action?.slice(0, 35)}...`)
  }
}
