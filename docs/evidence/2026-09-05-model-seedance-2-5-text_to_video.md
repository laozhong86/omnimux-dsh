---
title: "model evidence — seedance-2-5#text_to_video — 2026-09-05"
id: "evidence-model-seedance-2-5-text_to_video-20260905"
type: "evidence"
status: "accepted"
authority: "L3"
date: "2026-09-05"
...
# model evidence — seedance-2-5#text_to_video — 2026-09-05
...
## 2. minimal（最小生成）
- 最小输入：model=`seedance-2-5` op=`text_to_video`；prompt 固定短句；duration=5; resolution=720P; aspect_ratio=16:9；wire 字段=prompt-only
- 结果：submit HTTP 200; taskId `task_tgmrU2mITJAq6shZI5EMugC0xhUpPHU2`; terminal `success`; mode **live**; media bytes **4296075**; elapsedMs=195465
...
## 5. conclusion（结论）
- **当前 YAML 上架状态保持不变**：生成后 readback（2026-09-05T04:56:18.350Z）为 `HTTP 200 / video/mp4`，bytes 与 SHA-256 均匹配历史本地件，字段见 JSON。该 readback 是生成后的独立观测，不倒填为生成当时的响应头。
...
- logs：`docs/evidence/2026-09-05-model-seedance-2-5-text_to_video.json`
