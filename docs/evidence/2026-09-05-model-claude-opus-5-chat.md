---
title: "model evidence — claude-opus-5#chat — 2026-09-05"
id: "evidence-model-claude-opus-5-chat-20260905"
type: "evidence"
status: "accepted"
authority: "L3"
date: "2026-09-05"
...
# model evidence — claude-opus-5#chat — 2026-09-05

## 0. 身份（Identity）
| 字段 | 值 |
|---|---|
| runtime ID | `claude-opus-5`（wire alias：无） |
| operation | `chat` |
...
- 结果：HTTP 200；catalog 共 127 模型；`claude-opus-5` **在列**（present=true）
- 日志摘录：`missing=[]` for PR-A text set；totalListed=127

## 2. minimal（最小生成）
- 最小输入：`POST /v1/chat/completions` body `{model:"claude-opus-5", max_tokens:≤256, messages:[{role:user, content:"Reply with exactly one word: pong"}]}`（无 image）
- 结果：HTTP 200；mode 等价 **live**；response id `msg_a3b07857-a8d7-4b56-b7`；usage prompt=87 completion=3 total=90；内容摘要 `pong`；耗时 4661ms
...
- 备注：历史 2026-08-18 记录 chat-completions group 403；**本轮 token 45 对 `/v1/chat/completions` 已 200 live**（id `msg_a3b07857-a8d7-4b56-b7`），故翻 execution live。
