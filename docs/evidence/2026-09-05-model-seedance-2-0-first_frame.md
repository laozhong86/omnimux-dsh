...
- allowedMimes：PNG live via images[]; JPEG/WebP family-proven on fast; singular image rejected
...
## 5. conclusion（结论）
- **不接**：hub mapper emits singular `image` for first_frame; Seedance upstream rejects image with "seedance text-to-video does not support image"; live wire succeeds only with `images[]` (documented). Mapper/executor 本批禁止修改 → 不 listed。 证据仍落盘供复核；YAML 保持 draft/stub。
- logs：`docs/evidence/2026-09-05-model-seedance-2-0-first_frame.json`
