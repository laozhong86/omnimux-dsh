---
title: "model evidence — seedance-2-0-mini#text_to_video — 2026-09-05"
id: "evidence-model-seedance-2-0-mini-text_to_video-20260905"
type: "evidence"
status: "accepted"
authority: "L3"
date: "2026-09-05"
...
# model evidence — seedance-2-0-mini#text_to_video — 2026-09-05

## 0. 身份（Identity）
| 字段 | 值 |
|---|---|
| runtime ID | `seedance-2-0-mini`（wire alias：无） |
...
- 结果：HTTP 200；catalog 共 127 模型；`seedance-2-0-mini` **在列**（present=True）
- 日志摘录：seedance 相关 ID = seedance-2-0 / seedance-2-0-fast / seedance-2-0-mini / seedance-2-5 均 present=true（见同批 existence）

## 2. minimal（最小生成）
- 最小输入：model=`seedance-2-0-mini` op=`text_to_video`；prompt 固定短句；duration=4; resolution=720P; aspect_ratio=16:9；wire 字段=prompt-only
- 结果：submit HTTP 200; taskId `task_hOXqeQURbX4A0ICKjRjtiq0LD3lhtxJZ`; terminal `success`; mode **live**; media bytes **1560922**; elapsedMs=123876
...
| duration | parameter | 契约最小档实测成功 | 非法/过短：probe duration=0 → HTTP 200; body summary: {'created': 1788581528, 'id': 'task_aUcweKnF8ZntRjiXkwRkPnpvChUCppM6', 'model': 'seedance-2-0-mini', 'object': 'video', 'status': 'queued', 'task_id': 'task_aUcweKnF8ZntRjiXkwRkPnpvChUCppM6'} | 见 boundary 段；未即时拒绝者标 policy_conservative |
...
## 5. conclusion（结论）
- **当前 YAML 上架状态保持不变**：生成后 readback（2026-09-05T04:56:17.343Z）为 `HTTP 200 / video/mp4`，bytes 与 SHA-256 均匹配历史本地件，字段见 JSON。该 readback 是生成后的独立观测，不倒填为生成当时的响应头。
- artifact（历史生成）：terminal success 后本地保留 nonzero bytes；URL `.mp4` 与 ISO BMFF `ftypisom` 支持 `mimeInferred=video/mp4`；当前 HTTP/MIME readback 另见 JSON。
- logs：`docs/evidence/2026-09-05-model-seedance-2-0-mini-text_to_video.json`
