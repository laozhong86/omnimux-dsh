---
title: "model evidence — seedance-2-0-fast#first_frame — 2026-09-05"
id: "evidence-model-seedance-2-0-fast-first_frame-20260905"
type: "evidence"
status: "accepted"
authority: "L3"
date: "2026-09-05"
...
# model evidence — seedance-2-0-fast#first_frame — 2026-09-05
...
## 2. minimal（最小生成）
- 最小输入：model=`seedance-2-0-fast` op=`first_frame`；prompt 固定短句；duration=4; resolution=720P; aspect_ratio=16:9；wire 字段=images[]
- 结果：submit HTTP 200; taskId `task_6Uh074um1eu8uWw7xYH0Yp27yGkRZ9sn`; terminal `success`; mode **live**; media bytes **1640231**; elapsedMs=150001
- 日志摘录：脱敏摘要 JSON 见同 basename `.json`（request shape 不含 key/完整 data URI；媒体 URL 已去 query）
- extras：[{"file": "seedance-2-0-fast__first_frame_jpeg.json", "taskId": "task_qPwZBKMU0XsYMz2gA5Cc43kdRTb9QeSb", "mode": "live", "bytes": 1119401, "submitHttp": 200, "ok": true}, {"file": "seedance-2-0-fast__first_frame_webp_submit.json", "taskId": "task_EiiXD7mFxOExesO1sbudWYPjKckN3oYf", "mode": "submitted", "bytes": null, "submitHttp": 200, "ok": true}]
...
- allowedMimes：PNG+JPEG full live; WebP submit 200 task id (wait=false, 完整 generation 不记成功)
