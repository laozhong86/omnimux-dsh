...
> **2026-09-05 阶段一范围（当前有效）**：[视频模型阶段一范围：七个产品型号](2026-09-05-video-phase-one-scope.md) 取代本文 C1–C4/17 型号/派生 wire 的本阶段目标。本文其余内容只保留历史范围、架构背景和已发生事实；阶段一不含 Kling 全组、Wan 派生型号，MiniMax H3 按一个产品型号管理，其官方支持的多种模式仍须纳入接入核对。
