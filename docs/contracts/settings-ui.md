...
| A pinned first-level plugin's own page | product stage (`shell.overlay` + a stage store, opened directly from its own sidebar row) | Full-width product page over the conversation column, toggled from the pinned row (账号, 资产库, 专家·技能·连接器, 工作流, 灵感库) | The page is the plugin itself, not plugin config and not an inventory |
