...
Honest surface for both coding agents and the product agent. If a row is stub or absent, do not tell the user it works.
...
| `drama_generate_shot` | **real + stub** | live when `ctx.omnimuxVideo` is mounted (`mode: "live"`); else copies `stub.mp4` (`mode: "stub"`) |
| `omnimux_video_submit` | **real** | writes dest via OmniMux `/v1/video/generations`; needs `OMNIMUX_API_KEY` |
