...
2. Run `pnpm verify:models` (needs `OMNIMUX_API_KEY`; reads
   `~/.config/omnimux/dsh.env` when the env var is absent; self-skips without
   a key).
...
- `pnpm verify:models` — every declared model id exists on the live gateway.
