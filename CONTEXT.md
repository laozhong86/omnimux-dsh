...
This repo lands OmniMux on official dsh. `dsh-omnimux` is the execution hub, not a gateway. Verticals send requests into hub seams and write only their own disk. `omnimux-drama` is the first vertical (short-drama production). Drama Center upload is a different skill. I/O: `docs/contracts/hub.md`.
...
| `omnimux-drama` | `series/` contract and `drama_*` tools. Inputs generate via `videoGenerate` | Hub chrome, auth, model routes, OmniMux HTTP |
