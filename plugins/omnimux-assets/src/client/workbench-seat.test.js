...
describe('assets workbench seat (sidebar must not claim overlay)', () => {
  it('sidebar entry uses the workbench store, not the product stage', () => {
    const source = readFileSync(join(here, 'sidebar-entry.js'), 'utf8')
    assert.match(source, /createAssetsWorkbenchStore/)
    assert.match(source, /omnimux-assets:library/)
  ...
  it('client apply mounts the sidebar with a null product stage and registers tab', () => {
    ...
    assert.match(source, /id: ASSETS_TAB_ID/)
    assert.match(source, /registerAssetsTab/)
