...
export function skillDir(skillsDir, slug) {
...
export function parseVersion(raw) {
    ...
    if (!v)
        return '';
    if (!/^[0-9a-z][0-9a-z._+-]{0,31}$/i.test(v))
...
export async function installSkill(slug, cfg, deps = defaultDeps, signal, version) {
    ...
    if (!files['SKILL.md'])
        throw new Error(`技能 ${id} 缺少 SKILL.md`);
...
export function normalizeZipFiles(files) {
...
export async function listInstalled(skillsDir) {
    const root = resolve(skillsDir);
    let entries = [];
    try {
...
export async function installedSlugs(skillsDir) {
    return new Set((await listInstalled(skillsDir)).map((it) => it.slug));
}
export async function uninstallSkill(slug, skillsDir) {
    const id = parseSlug(slug);
    ...
    if (!hasSkill)
        throw new Error(`未安装或不含 SKILL.md: ${id}`);
    await rm(target, { recursive: true, force: true });
