...
export function renderPluginSearch(value) {
    if (!value.items?.length)
        return '没有找到相关 DSH 插件。对用户只说一句：没找到，可以换个词再搜。不要写长文。不要打印安装命令。';
...
export function renderPluginInstall(value) {
    return `${value.fullName || '插件'} 已安装。需要用户自行重启 Host 后生效。不要声称已重启。不要打印安装命令。`;
}
export function renderPluginUninstall(value) {
    return `${value.name || '插件'} 已卸载。需要用户自行重启 Host。不要声称已重启。不要打印卸载命令，不要建议用 bash。`;
}
export function renderPluginList(value) {
    if (!value.items?.length)
        return '当前 profile 没有已装插件。不要教用户卸载核心包。不要打印安装命令。';
...
export function renderConnectorSearch(value) {
    if (!value.items?.length)
        return '没有找到相关连接器。对用户只说一句：没找到。不要写长文。不要当专家推。不要打印安装命令。';
    const lines = value.items.map((it, i) => {
...
export function renderConnectorInstall(value) {
    return `连接器 ${value.id || ''} 已安装。需要用户自行重启 Host。不要声称已重启。不要打印安装命令。`;
}
export function renderConnectorUninstall(value) {
...
export function renderConnectorList(value) {
    ...
    const market = value.marketplaceDisplayOnly
        ...
        : '';
    if (!n)
        return `还没有已装连接器。${market}对用户最多一句。不要写长文。`;
