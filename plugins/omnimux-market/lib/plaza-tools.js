 ...
 */
export function tokenizePlazaQuery(query) {
...
function scorePlazaItem(item, tokens) {
    if (!tokens.length)
        return 1;
 ...
 */
export function listPlazaLocal(catalog, query, tab, limit) {
    ...
    const pool = catalog.items.filter((item) => {
        ...
        if (!PLAZA_KINDS.has(kind))
            return false;
        if (wantTab && String(item.tab || '') !== wantTab)
            return false;
        return true;
    });
...
function renderPlazaSummon(value) {
    ...
    return [
        {
            ...
            output: {
                ...
                render: (_args, value) => [{ type: 'text', text: renderPlazaSearch(value) }],
                presentationMeta: (_args, value) => ({ kind: 'plaza-search', ...value }),
            },
            presentCall: (args) => ({
                card: 'generic',
            ...
            async execute(args, exec) {
                ...
                const doc = decorateCatalog(loadCatalog(), roots());
        ...
        },
        {
            ...
            output: {
                schema: { type: 'object', additionalProperties: true },
                render: (_args, value) => [{ type: 'text', text: renderPlazaSummon(value) }],
                presentationMeta: (_args, value) => ({ kind: 'plaza-summon', ...value }),
            },
            ...
            async execute(args, exec) {
                ...
                if (item.kind === 'connector')
                    throw new Error('connectors are installed, not summoned');
                if (!PLAZA_KINDS.has(String(item.kind || '')))
                    throw new Error(`${id} is not an expert or team`);
                const r = roots();
        ...
        },
        {
            ...
            output: {
                ...
                presentationMeta: (_args, value) => ({ kind: 'plaza-install', ...value }),
            },
