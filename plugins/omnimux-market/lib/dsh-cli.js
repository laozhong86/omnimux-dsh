...
export function preparePluginArgs(profileDirectory, pluginArgs) {
    const args = pluginArgsFor(profileDirectory, pluginArgs);
    const command = args[0];
    if (command !== undefined && NDJSON_COMMANDS.has(command))
        return [...args, '--reporter=ndjson'];
    return args;
}
...
export function writeDangerouslyAllowAllBuilds(profileDirectory) {
...
export async function addDshPlugin(source, deps = {}) {
    ...
    const retryAfterPrepare = async (err) => {
        const text = err instanceof Error ? err.message : String(err);
        if (!isPrepareBlocked(text))
