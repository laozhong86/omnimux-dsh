...
test('fetchInstallPlan hits install-plan URL', async () => {
    const cfg = withDefaults({ apiBase: 'https://api.skillhub.cn/' });
    const body = await fetchInstallPlan(cfg, 'o', 'n', async (url) => {
        assert.equal(url, 'https://api.skillhub.cn/api/v1/plugins/o/n/install-plan');
        return { source: 'github:o/n#abcdef0' };
    });
    assert.equal(body.source, 'github:o/n#abcdef0');
});
...
test('installMarketPlugin does not spawn when the plan is for another repo', async () => {
    const cfg = withDefaults({ apiBase: 'https://api.skillhub.cn' });
    await assert.rejects(() => installMarketPlugin({ owner: 'o', name: 'n' }, cfg, {
        fetchJson: async () => ({ source: 'github:evil/n#abcdef0' }),
        runDshPlugin: async () => {
...
test('installPlanUrl encodes owner/name', () => {
    assert.equal(installPlanUrl('https://api.skillhub.cn', 'o', 'n'), 'https://api.skillhub.cn/api/v1/plugins/o/n/install-plan');
});
...
test('listPlugins maps catalog payload and returns webBase', async () => {
    ...
    let seen = '';
    const page = await listPlugins(cfg, { q: 'mod', scope: 'verified', pageSize: 24 }, async (url) => {
        seen = url;
        return {
            total: 1,
            page: 1,
...
test('listPlugins marks plugins already in the profile', async () => {
    const cfg = withDefaults({ apiBase: 'https://api.skillhub.cn' });
    const page = await listPlugins(cfg, {}, async () => ({
        items: [
...
test('listPluginCategories uses catalog payload and falls back', async () => {
    const cfg = withDefaults({ apiBase: 'https://api.skillhub.cn' });
    const items = await listPluginCategories(cfg, async (url) => {
        assert.equal(url, 'https://api.skillhub.cn/api/v1/plugins/categories');
        return { items: [{ key: 'web-tools', displayName: '联网工具' }, { key: 'office-efficiency', displayName: 'nope' }] };
    });
