...
import { PLUGIN_GITHUB_SPEC, compareSemver, detectSource, fetchLatestRelease, getUpdateStatus, normalizeVersion, parseReleaseTag, updateToLatestRelease, } from '../self-update.js';
...
test('getUpdateStatus marks github install up to date', async () => {
    const status = await getUpdateStatus({ timeoutMs: 1000, userAgent: 't' }, {
        fetchJson: async () => ({ tag_name: 'v0.1.0' }),
        runDshPlugin: async () => '',
