...
import { HttpError, clearHttpJsonCache, configureHttpJsonCache, fetchBytes, fetchJson, httpJsonCacheSize, resetHttpNow, setHttpNow, } from '../http.js';
...
test('fetchJson TTL memo hits same URL without a second network call', async () => {
    ...
    try {
        const a = await fetchJson('https://example.test/memo', opts);
        const b = await fetchJson('https://example.test/memo', opts);
        assert.equal(a.n, 1);
        assert.equal(b.n, 1);
        assert.equal(calls, 1);
        assert.equal(httpJsonCacheSize(), 1);
        now += 91_000;
        const c = await fetchJson('https://example.test/memo', opts);
        assert.equal(c.n, 2);
        assert.equal(calls, 2);
        const d = await fetchJson('https://example.test/memo', { ...opts, refresh: true });
        assert.equal(d.n, 3);
...
test('HTTP 404 becomes HttpError with status', async () => {
    ...
    try {
        await assert.rejects(() => fetchJson('https://example.test/missing', opts), (err) => {
...
test('aborted fetch is reported as timeout', async () => {
    ...
    try {
        await assert.rejects(() => fetchJson('https://example.test/slow', { timeoutMs: 20, userAgent: 't' }), /timeout 20ms/);
    }
    finally {
        globalThis.fetch = orig;
    }
...
test('network failure is wrapped in HttpError', async () => {
    ...
    try {
        await assert.rejects(() => fetchJson('https://example.test/down', opts), (err) => {
            assert.ok(err instanceof HttpError);
