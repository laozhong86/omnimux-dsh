...
test('withDangerouslyAllowAllBuilds is idempotent and flips false to true', () => {
    ...
    assert.equal(withDangerouslyAllowAllBuilds('packages:\n  - .\n'), 'packages:\n  - .\n\ndangerouslyAllowAllBuilds: true\n');
    const allowed = 'packages:\n  - .\n\ndangerouslyAllowAllBuilds: true\n';
    assert.equal(withDangerouslyAllowAllBuilds(allowed), allowed);
    assert.equal(withDangerouslyAllowAllBuilds('dangerouslyAllowAllBuilds: false\n'), 'dangerouslyAllowAllBuilds: true\n');
...
test('addDshPlugin retries once after a hoist-pattern failure', async () => {
    ...
    const log = await addDshPlugin('github:o/n#abcdef0', {
        runDshPlugin: async (profile, args) => {
            ...
            n += 1;
            if (n === 1)
                throw new Error('ERR_PNPM_PUBLIC_HOIST_PATTERN_DIFF in modules');
            return 'rebuilt';
        },
    });
    ...
    try {
        ...
        const log = await addDshPlugin('github:o/n#abcdef0', {
            ...
            runDshPlugin: async (profile, args) => {
                ...
                n += 1;
                if (n === 1)
                    throw new Error('ERR_PNPM_GIT_DEP_PREPARE_NOT_ALLOWED needs to execute build scripts');
                return 'allowed';
            },
        });
...
test('addDshPlugin rewrites leftover hoist and prepare errors', async () => {
    await assert.rejects(() => addDshPlugin('github:o/n#abcdef0', {
        ...
        const log = await addDshPlugin('github:o/n#abcdef0', {
            ...
            runDshPlugin: async (_profile, args) => {
                ...
                if (adds === 1)
                    throw new Error('ERR_PNPM_PUBLIC_HOIST_PATTERN_DIFF in modules');
                if (adds === 2)
                    throw new Error('ERR_PNPM_GIT_DEP_PREPARE_NOT_ALLOWED');
                return 'allowed';
            },
        });
        assert.equal(log, 'allowed');
...
test('runCommand captures stdout and rejects non-zero exit', async () => {
    ...
    await assert.rejects(() => runCommand(process.execPath, ['-e', 'process.exit(2)'], { cwd: process.cwd(), timeoutMs: 5000 }), /exit 2/);
});
test('runCommand times out and can be aborted', async () => {
    await assert.rejects(() => runCommand(process.execPath, ['-e', 'setTimeout(() => {}, 30_000)'], { cwd: process.cwd(), timeoutMs: 80 }), /命令超时/);
    await assert.rejects(() => runCommand(process.execPath, ['-e', 'setTimeout(() => {}, 30_000)'], {
        cwd: process.cwd(),
        timeoutMs: 80,
        detached: true,
    }), /命令超时/);
...
test('runCommand surfaces stderr and spawn errors', async () => {
    await assert.rejects(() => runCommand(process.execPath, ['-e', "process.stderr.write('boom'); process.exit(1)"], {
        cwd: process.cwd(),
        timeoutMs: 5000,
    }), /boom/);
    await assert.rejects(() => runCommand('/no/such/skillhub-dsh-binary', [], { cwd: process.cwd(), timeoutMs: 1000 }), /ENOENT|no such|not found/i);
});
...
test('runCommand forwards stdout and stderr chunks to onChunk', async () => {
    const seen = [];
    const out = await runCommand(process.execPath, ['-e', "process.stdout.write('chunk-a'); process.stderr.write('chunk-b')"], {
        cwd: process.cwd(),
        timeoutMs: 5000,
