...
import { readProcCgroup, respawnInvocation, restartHelperSource, restartLaunch, scheduleRestart, servingPort, systemdRestartArgv, systemdUnitName, trustedRestartRequest, } from '../restart.js';
...
test('trustedRestartRequest accepts same-origin UI clicks, including reverse proxies', () => {
    ...
    assert.equal(trustedRestartRequest({ headers: ok, socket: {} }), true);
});
...
test('restartHelperSource waits for the port and names skillhub in logs', () => {
    const src = restartHelperSource({ file: 'node', args: ['bin.js'], viaShell: false, detached: true }, { cwd: '/tmp' }, { out: '/tmp/out.log', err: '/tmp/err.log' }, 3080);
    assert.match(src, /\[skillhub\]/);
    assert.match(src, /port/);
    assert.match(src, /3080/);
    assert.match(src, /net\.connect/);
    const noPort = restartHelperSource({ file: 'node', args: ['bin.js'], viaShell: false, detached: true }, { cwd: '/tmp' }, { out: '/tmp/out.log', err: '/tmp/err.log' }, null);
...
test('systemdUnitName reads the innermost service and skips user@', () => {
    ...
    assert.equal(systemdUnitName('1:name=systemd:/system.slice/deepseek-harness.service'), 'deepseek-harness.service');
    assert.equal(systemdUnitName('0::/user.slice/user-1000.slice/user@1000.service/app.slice/dsh.service'), 'dsh.service');
