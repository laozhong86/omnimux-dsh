...
import { addDshPlugin, webProfileDir } from './dsh-cli.js';
...
export function parsePluginRef(owner, name) {
    const o = String(owner || '').trim();
    const n = String(name || '').trim();
    if (!OWNER_RE.test(o))
        throw new Error('无效插件 owner');
    if (!NAME_RE.test(n))
        throw new Error('无效插件 name');
    return { owner: o, name: n, fullName: `${o}/${n}` };
}
export function parsePluginCategory(raw) {
    const key = String(raw || '').trim();
    if (!key)
        return undefined;
    return PLUGIN_CATEGORY_KEYS.includes(key) ? key : undefined;
}
export function sanitizePluginScope(raw) {
    return raw === 'all' ? 'all' : 'verified';
}
export function sanitizePluginSort(raw) {
    return raw === 'updated' ? 'updated' : 'stars';
}
function pluginPageSize(raw) {
    const n = Math.floor(Number(raw) || 24);
    return Math.max(1, Math.min(100, n));
}
function pluginPage(raw) {
...
export function installPlanUrl(apiBase, owner, name) {
...
export function pluginCategoriesUrl(apiBase) {
    return `${trimBase(apiBase)}/api/v1/plugins/categories`;
}
export function sanitizePluginAvatarUrl(raw) {
    const url = String(raw || '').trim();
    if (!/^https:\/\//i.test(url) || url.length > 500 || /[\s<>"'`]/.test(url))
        return '';
    return url;
}
export function buildPluginsUrl(apiBase, query = {}) {
...
export function mapMarketPlugin(raw) {
...
export function githubRepoFromSpec(spec) {
    const matched = GITHUB_REPO.exec(String(spec || '').trim());
    if (!matched)
        return null;
    return matched[1].replace(/\.git$/i, '').toLowerCase();
}
export function readInstalledPlugins(profileDir = webProfileDir()) {
    try {
        const raw = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8'));
...
export function isMarketPluginInstalled(plugin, installed) {
...
function packageNameMatches(pkg, plugin) {
    const lower = pkg.toLowerCase();
    const name = plugin.name.toLowerCase();
    if (lower === name)
        return true;
    if (lower === `@${plugin.owner.toLowerCase()}/${name}`)
        return true;
...
export function resolveInstallSource(raw, expected) {
    if (!raw || typeof raw !== 'object')
        throw new Error('安装计划无效');
    const plan = raw;
    ...
    if (!source) {
        ...
        if (!matched)
            throw new Error('安装计划没有可用的 source');
        source = matched[1];
    }
    const pin = GITHUB_PIN.exec(source);
    if (!pin)
        throw new Error('安装计划 source 不是 pinned github 规格');
    const repo = pin[1];
    const sha = pin[2];
    ...
    const plugin = plan.plugin && typeof plan.plugin === 'object' ? plan.plugin : {};
    ...
    if (headSha && headSha.toLowerCase() !== sha.toLowerCase()) {
        ...
        if (!a.startsWith(b) && !b.startsWith(a))
            throw new Error('安装计划 commit 与 source 不一致');
    }
    const repoUrl = String(plugin.repositoryUrl || '').trim();
    if (repoUrl) {
        const ok = new RegExp(`^https://github\\.com/${escapeRegExp(expected.owner)}/${escapeRegExp(expected.name)}(?:\\.git)?/?$`, 'i').test(repoUrl);
        if (!ok)
            throw new Error('安装计划仓库地址与所选插件不一致');
    }
    return source;
}
export async function fetchInstallPlan(cfg, owner, name, fetchJsonImpl = fetchJson) {
    const ref = parsePluginRef(owner, name);
    return fetchJsonImpl(installPlanUrl(cfg.apiBase, ref.owner, ref.name), fetchOpts(cfg));
}
export async function installMarketPlugin(plugin, cfg, deps = {}) {
...
export function fallbackPluginCategories() {
    return PLUGIN_CATEGORY_KEYS.map((key) => ({ key, displayName: PLUGIN_CATEGORIES[key] }));
}
export function mapPluginCategory(raw) {
    if (!raw || typeof raw !== 'object')
        return null;
    const r = raw;
...
export async function listPluginCategories(cfg, fetchJsonImpl = fetchJson) {
    const url = pluginCategoriesUrl(cfg.apiBase);
    try {
        const body = await fetchJsonImpl(url, fetchOpts(cfg));
        const items = (Array.isArray(body.items) ? body.items : []).map(mapPluginCategory).filter((it) => !!it);
...
export async function listPlugins(cfg, query = {}, fetchJsonImpl = fetchJson, installedMap = readInstalledPlugins()) {
