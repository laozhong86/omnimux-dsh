...
export function unzipToFiles(buf) {
    const eocd = findEocd(buf);
    return eocd >= 0 ? unzipFromCentral(buf, eocd) : unzipFromLocal(buf);
}
function unzipFromCentral(buf, eocd) {
...
function readEntry(buf, localOff, method, compSize, name) {
...
function inflateEntry(compressed, method, name) {
    if (method === 0)
