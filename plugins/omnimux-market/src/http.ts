...
export type JsonFetchOptions = FetchOptions & {
  /** Bypass TTL memo when true, or when the URL carries `refresh=1`. */
  refresh?: boolean
  /** Per-call TTL override (ms). `0` disables caching for this call. */
  cacheTtlMs?: number
}
...
export function setHttpNow(fn: () => number): void {
  nowMs = fn
}

export function resetHttpNow(): void {
  nowMs = () => Date.now()
}

export function configureHttpJsonCache(opts: { ttlMs?: number } = {}): void {
  if (opts.ttlMs != null && Number.isFinite(opts.ttlMs) && opts.ttlMs >= 0) {
    defaultTtlMs = Math.floor(opts.ttlMs)
  }
}

export function clearHttpJsonCache(): void {
  jsonCache.clear()
}

export function httpJsonCacheSize(): number {
...
function wantsRefresh(url: string, options: JsonFetchOptions): boolean {
...
export async function fetchJson<T>(url: string, options: JsonFetchOptions, signal?: AbortSignal): Promise<T> {
  ...
  if (!refresh && ttl > 0) {
    ...
    if (hit && nowMs() - hit.at < ttl) return hit.value as T
  }
  const value = await fetchJsonUncached<T>(url, options, signal)
  if (ttl > 0) jsonCache.set(key, { at: nowMs(), value })
  return value
}

/** Direct network path used by tests that need to assert cache bypasses. */
export async function fetchJsonUncached<T>(url: string, options: FetchOptions, signal?: AbortSignal): Promise<T> {
