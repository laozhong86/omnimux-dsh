...
export type MarketplaceConnectorCard = {
  id: string
  slug: string
  name: string
  description: string
  ...
  kind: 'connector'
  gesture: string
  /** P0：广场只展示，不走 catalogInstall */
  installable: boolean
  sourceKind: 'marketplace'
  connectorType: 'mcp' | 'cli' | 'skill-only'
}
...
const TYPE_META: Record<'mcp' | 'cli' | 'skill-only', { id: string, title: string }> = {
 ...
 */
export function resolveMarketplaceRoot(home = homedir()): string {
  if (process.env.WORKBUDDY_CONNECTORS_MARKETPLACE) return process.env.WORKBUDDY_CONNECTORS_MARKETPLACE
  return join(home, '.workbuddy', 'connectors-marketplace')
}

export function marketplaceManifestPath(root = resolveMarketplaceRoot()): string {
  return join(root, '.codebuddy-connector', 'connectors.json')
}

export function marketplaceIconsDir(root = resolveMarketplaceRoot()): string {
...
const ICON_EXTS = ['.svg', '.png', '.webp', '.jpg', '.jpeg', '.gif'] as const

type ListCache = {
  mtimeMs: number
  size: number
  items: MarketplaceConnectorCard[]
  categories: MarketplaceConnectorCategory[]
}

/** root → memoized list (invalidated when manifest mtime/size changes) */
const listMemo = new Map<string, ListCache>()
/** `${root}\0${id}` → marketplace-icon:<id> or '' */
const iconUrlMemo = new Map<string, string>()

export function clearMarketplaceConnectorMemos(): void {
  listMemo.clear()
  iconUrlMemo.clear()
}

export function marketplaceListMemoSize(): number {
 ...
 */
export function sanitizeMarketplaceIconId(raw: string): string {
 ...
 */
export function resolveMarketplaceIconUrl(id: string, root = resolveMarketplaceRoot()): string {
  const safeId = sanitizeMarketplaceIconId(id)
  if (!safeId) return ''
  const key = `${root}\0${safeId}`
  if (iconUrlMemo.has(key)) return iconUrlMemo.get(key) || ''
...
export function resolveMarketplaceIconFile(
  iconRef: string,
  root = resolveMarketplaceRoot(),
): { path: string, contentType: string } | null {
  if (!iconRef.startsWith(MARKETPLACE_ICON_SCHEME)) return null
...
function normalizeType(raw: unknown): 'mcp' | 'cli' | 'skill-only' {
  if (raw === 'cli') return 'cli'
  if (raw === 'skill-only') return 'skill-only'
  return 'mcp'
}
...
export function listMarketplaceConnectors(root = resolveMarketplaceRoot()): {
  ...
  const usedTypes = new Set<'mcp' | 'cli' | 'skill-only'>()
  const items: MarketplaceConnectorCard[] = []
  ...
  const categories = (['mcp', 'cli', 'skill-only'] as const)
    .filter((id) => usedTypes.has(id))
    .map((id) => ({ id: TYPE_META[id].id, title: TYPE_META[id].title, tab: 'connectors' as const }))
