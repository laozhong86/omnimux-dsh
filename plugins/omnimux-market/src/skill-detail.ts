...
const evalScoreMemo = new Map<string, { at: number, score: number | null }>()
const EVAL_TTL_MS = 20 * 60 * 1000
let evalNow = () => Date.now()

/** Test hook. */
export function setEvalNow(fn: () => number): void {
  evalNow = fn
}

export function resetEvalNow(): void {
  evalNow = () => Date.now()
}

export function clearEvalScoreMemo(): void {
  evalScoreMemo.clear()
}

export function evalScoreMemoSize(): number {
...
export async function fetchEvalScore(
  ...
  if (hit && evalNow() - hit.at < EVAL_TTL_MS) return hit.score
  try {
