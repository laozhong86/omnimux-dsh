...
test('renderAttachedExpertSection escapes double braces {{...}} for DSH interpolate safety', () => {
  ...
  writeFileSync(
    join(h, 'skills', 'dev-ops-automation-engineer', 'SKILL.md'),
  ...
  const GROUP_AT = /^\{\{([^{}]*)\}\}/
  function mockDshInterpolate(input: { text: string; name: string }, variables: Record<string, string>) {
    ...
    for (let open = raw.indexOf('{{'); open >= 0; open = raw.indexOf('{{', last)) {
      ...
      if (group === null) {
        if (raw.indexOf('}}', open + 2) >= 0) throw new Error(`malformed prompt variable reference in ${input.name}`)
        result += raw.slice(last, open + 2)
        last = open + 2
        continue
      }
      const name = group[0].slice(2, -2)
      if (!VARIABLE_NAME.test(name)) throw new Error(`malformed prompt variable reference "{{${name}}}" in ${input.name}`)
      if (!Object.hasOwn(variables, name)) throw new Error(`unknown prompt variable "{{${name}}}" in ${input.name}`)
