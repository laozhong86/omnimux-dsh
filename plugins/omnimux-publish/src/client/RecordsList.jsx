...
export function RecordsList({ t, tab, records, loading, onRefresh, onOpen, onEdit, onDeleted }) {
  ...
  if (loading) {
    return <div className="omnimux-publish-muted">{t('loading')}</div>
  }
  ...
  return (
    <div>
      <div className="omnimux-publish-list-head">
        <Button variant="ghost" size="sm" leadingIcon={<IconRefreshOutline16 />} onClick={onRefresh}>
          {t('records.refresh')}
        </Button>
      </div>
      {error ? (
        <div role="alert" className="omnimux-publish-alert error">{error}</div>
      ) : null}
      <div className="omnimux-publish-cards">
        {records.map((record) => (
          <RecordCard
...
function RecordCard({ t, record, busy, onOpen, onEdit, onDelete }) {
  ...
  return (
    <div
      className={busy ? 'omnimux-publish-card busy' : 'omnimux-publish-card'}
      onClick={() => { if (!busy) (isDraft ? onEdit : onOpen)(record) }}
    >
      <div className="omnimux-publish-thumbs">
        {mediaIds.slice(0, 3).map((mediaId) => (
          <Thumb key={String(mediaId)} mediaId={String(mediaId)} />
        ))}
        {mediaIds.length === 0 ? <div className="omnimux-publish-thumb xs"><IconEditOutline16 /></div> : null}
      </div>
      <div className="omnimux-publish-card-top">
        <span className="omnimux-publish-card-title">
          {String(record.title || record.description || '(untitled)').slice(0, 40)}
        </span>
        <StatusChip t={t} aggregate={aggregate} />
      </div>
      <div className="omnimux-publish-meta">
      ...
      {isDraft ? null : (
        <div className="omnimux-publish-sum">
      ...
      {record.error ? (
        <div className="omnimux-publish-card-err">{String(record.error)}</div>
      ) : null}
      {isDraft ? (
        <div className="omnimux-publish-card-foot">
          <Button
            type="button"
            size="xs"
            variant="danger"
...
function Thumb({ mediaId }) {
  ...
  return (
    <img
      ...
      alt=""
      loading="lazy"
      className="omnimux-publish-thumb xs"
      onError={(event) => {
...
function StatusChip({ t, aggregate }) {
  return (
    <span className="omnimux-publish-chip" data-agg={aggregate}>
      {t(`agg.${aggregate}`)}
    </span>
  )
}
...
function EmptyState({ t, tab }) {
  ...
  return (
    <div className="omnimux-publish-empty">
      <div className="omnimux-publish-empty-icon"><IconArchiveOutline20 /></div>
      <div className="omnimux-publish-empty-title">{title}</div>
      <div className="omnimux-publish-empty-hint">{hint}</div>
    </div>
  )
}
