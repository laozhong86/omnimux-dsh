...
export async function execute(input, ctx) {
  ...
  const pythonBin = ctx.videoConfig?.video?.pythonPath || process.env.DSH_VIDEO_PYTHON_PATH || 'python3'
  const modelsDir = ctx.videoConfig?.video?.modelsDir || process.env.DSH_VIDEO_MODELS_DIR || ''
