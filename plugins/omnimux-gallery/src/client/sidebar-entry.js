/**
 * 专家馆 (Experts · Skills · Connectors) row under 新会话, placed by the
 * single sidebar coordinator (window.__omnimuxSidebar from the omnimux hub).
 * The coordinator owns all observers and ordering so multiple plugins cannot
 * re-place rows into each other (the dead-loop root cause). This module only
 * describes the row and its label/click behavior.
 */
...
export function mountSidebarEntry(gallery, t, locale) {
  ...
  const unsubscribeGallery = gallery.subscribe(syncActive)
  syncActive()

  const unregister = registerWhenReady({
    id: 'omnimux-gallery-entry',
    rank: 3,
    styles: STYLES,
    styleId: 'omnimux-esc-entry-styles',
    create: () => entry,
  })
