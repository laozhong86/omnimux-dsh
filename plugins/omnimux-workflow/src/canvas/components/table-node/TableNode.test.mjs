...
test('TableNode 顶部胶囊栏契约：100% 复用通用 FloatingTopPill 组件，收敛为两个纯图标 Action（添加到对话、全屏编辑），移除添加行按钮', () => {
  // 1. 引用并消费全仓通用的 FloatingTopPill 抽象组件
  ...
  // 2. 包含「添加到对话」与「全屏编辑」两个纯图标 Action 配置项及 title 提示
  assert.match(tableNodeSrc, /key:\s*'add-to-chat'/);
  assert.match(tableNodeSrc, /icon:\s*MessageSquarePlus/);
  assert.match(tableNodeSrc, /title:\s*'添加到对话'/);
