...
test('collectRuntimeModelIds returns the 43-id universe (contracts + wire aliases)', () => {
  resetContractCache();
  const ids = collectRuntimeModelIds();
  assert.equal(ids.length, 43, `expected 43 runtime ids, got ${ids.length}`);
...
test('verifyContracts: audit ok; strict ok once 43 dispositions resolve', () => {
