/**
 * Dispositions registry tests (H2 + C1): 45-row lock, shape validation, D1-D7
...
test('dispositions.json: exactly 45 rows, unique ids, all kinds valid, reasons present', () => {
  resetDispositionsCache();
  const doc = loadDispositions();
  assert.equal(validateDispositionsShape(doc).length, 0);
  const rows = doc.dispositions;
  assert.equal(rows.length, 45, `expected 45 disposition rows, got ${rows.length}`);
...
test('45 disposition rows mirror the runtime universe exactly (no missing, no ghost)', () => {
