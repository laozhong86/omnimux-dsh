  ...
  apps: [
    {
      ...
      client: true,
      spec: { source: /** @type {'npm'} */ ('npm'), name: 'dsh-omnimux-accounts', version: '0.1.0' },
    },
    {
      id: 'hidden',
      title: '隐藏',
...
describe('buildInstalledAppsView', () => {
  it('does not invent cards from unmarked profile bundles', () => {
    const view = buildInstalledAppsView({
      ...
      home: '/tmp',
      profile: 'omnimux',
      readBundles: () => [{ name: 'dsh-cron-parse' }, { name: 'dsh-omnimux' }],
      readVersion: () => '1.0.0',
    })
    assert.deepEqual(view.apps.map((app) => app.id), ['accounts'])
    assert.equal(view.apps[0].state, 'available')
  ...
  it('reads the installed version from the profile node_modules tree', () => {
    ...
    const pkg = join(home, 'profiles', 'omnimux', 'node_modules', 'dsh-omnimux-accounts')
    mkdirSync(pkg, { recursive: true })
    writeFileSync(join(pkg, 'package.json'), JSON.stringify({ name: 'dsh-omnimux-accounts', version: '0.1.0' }))
    assert.equal(readInstalledVersion(home, 'omnimux', 'dsh-omnimux-accounts'), '0.1.0')
    const view = buildInstalledAppsView({
      catalog,
      home,
      profile: 'omnimux',
      readBundles: () => [{ name: 'dsh-omnimux-accounts' }],
    })
