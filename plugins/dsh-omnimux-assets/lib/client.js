...
function ensureProductStageChrome() {
  const existing = document.getElementById("dsh-omnimux-assets-stage-chrome");
  if (existing instanceof HTMLStyleElement) {
    if (!existing.textContent?.includes("dsh-window-drag")) existing.textContent = PRODUCT_STAGE_CHROME;
  } else {
    const style = document.createElement("style");
    style.id = "dsh-omnimux-assets-stage-chrome";
...
var STAGE_ID = "dsh-omnimux-assets";
...
.dsh-omnimux-assets-entry:hover { background: var(--dsw-alias-interactive-bg-hover, rgba(128,128,128,.12)); }
.dsh-omnimux-assets-entry[data-active="true"] { background: var(--dsw-alias-interactive-bg-active, rgba(128,128,128,.18)); font-weight: 500; }
.dsh-omnimux-assets-entry-icon { flex: none; display: inline-flex; width: 14px; height: 14px; align-items: center; justify-content: center; }
.dsh-omnimux-assets-entry svg { display: block; width: 14px; height: 14px; }
.dsh-omnimux-assets-entry-label { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; line-height: 20px; }
`;
function injectStyles() {
  if (document.getElementById("dsh-omnimux-assets-entry-styles")) return;
  const style = document.createElement("style");
  style.id = "dsh-omnimux-assets-entry-styles";
...
function paintLabel(entry, label3) {
  ...
  const node = entry.querySelector(".dsh-omnimux-assets-entry-label");
...
function createEntry(stage, t) {
  ...
  entry.className = "dsh-omnimux-assets-entry";
  entry.innerHTML = `<span class="dsh-omnimux-assets-entry-icon">${ICON}</span><span class="dsh-omnimux-assets-entry-label"></span>`;
...
// src/client/index.js
var name = "dsh-omnimux-assets";
var inject = ["slots", "locale"];
function apply(ctx) {
  ctx.effect(() => ctx.locale.register(NS, { zh, en }), "dsh-omnimux-assets: dictionaries");
  const t = ctx.locale.bind(NS);
  const stage = createStageStore();
  const stageFace = () => ({ t, stage });
  ctx.effect(() => mountSidebarEntry(stage, t, ctx.locale), "dsh-omnimux-assets: sidebar entry");
  ctx.slots.inject("shell.overlay", () => ctx.slots.register({
    name: "shell.overlay",
    id: "dsh-omnimux-assets-stage",
    order: 30,
    locale: NS,
    inject: stageFace
  }, AssetsStage));
