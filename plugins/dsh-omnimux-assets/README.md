...
```sh
dsh plugin --profile omnimux add /Users/x/Desktop/Project/dsh-plugin/product/omnimux-dsh/plugins/dsh-omnimux-assets
dsh --profile omnimux --dump-config   # 应出现 dsh-omnimux-assets
...
npm run build # esbuild → lib/client.js（ModuleLoader 包裹，ID = dsh-omnimux-assets）
```
...
- 不 import hub（`dsh-omnimux`）任何内部模块：chrome CSS、loopback 写校验、sendJson 均为本插件内等义自实现（id 隔离）。
