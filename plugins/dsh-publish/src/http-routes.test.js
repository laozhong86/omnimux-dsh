...
describe('registerPublishRoutes: 前缀路由全家', () => {
  ...
  it('GET /dsh-publish/state answers revision and cheap polling', async () => {
    const first = await call({ method: 'GET', url: '/dsh-publish/state' })
    ...
    const same = await call({ method: 'GET', url: `/dsh-publish/state?rev=${first.body.rev}` })
    assert.equal(same.body.unchanged, true)
  })

  it('GET /dsh-publish/records filters the three tabs', async () => {
    freshDispatcher()
    dispatcher.createDraft({ type: 'image', payload: { title: '草稿', description: '描述' } })
    const result = await call({ method: 'GET', url: '/dsh-publish/records?status=draft' })
    assert.equal(result.status, 200)
    assert.equal(result.body.records.length, 1)
    assert.equal(result.body.records[0].title, '草稿')
  })

  it('GET /dsh-publish/capabilities serves the merged matrix (表单裁剪数据源)', async () => {
    const result = await call({ method: 'GET', url: '/dsh-publish/capabilities' })
    assert.equal(result.status, 200)
