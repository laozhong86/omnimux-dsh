...
export function Composer({ t, draftId, onBack, onSubmitted, onSaved }) {
  ...
  if (!type) {
    return (
      <div>
        ...
        <div className="dsh-pub-type-pick">
          <div className="dsh-pub-type-title">{t('type.title')}</div>
          <div className="dsh-pub-type-row">
  ...
  return (
    <div className="dsh-pub-composer">
      <AccountPanel
        t={t}
      ...
      <div className="dsh-pub-form">
        <Toolbar
          ...
          left={(
            <>
              ...
              {editingId ? <span className="dsh-pub-lock">{t('type.locked')}</span> : null}
            </>
          )}
          right={(
            <>
        ...
        {error ? <div role="alert" className="dsh-pub-alert error">{error}</div> : null}
        {notice ? <div role="status" className="dsh-pub-alert ok">{notice}</div> : null}
        ...
        <label className="dsh-pub-field">
          <span className="dsh-pub-label">{t('form.description')}</span>
          <textarea
            value={description}
        ...
        <div className="dsh-pub-field">
          <span className="dsh-pub-label">{t('form.media')}</span>
          <div className="dsh-pub-media">
            {mediaList.map((media) => (
              <div key={media.id} className="dsh-pub-media-item">
                {media.kind === 'image' ? (
                  <img src={mediaContentUrl(media.id)} alt={media.filename} className="dsh-pub-thumb" />
                ) : (
                  <div className="dsh-pub-thumb"><IconPlayOutline16 size={20} /></div>
                )}
                <div className="dsh-pub-media-actions">
        ...
        {type === 'image' ? (
          <div className={coverBlocked ? 'dsh-pub-field dim' : 'dsh-pub-field'}>
            <span className="dsh-pub-label">{t('form.cover')}</span>
            <div className="dsh-pub-cover-row">
              {coverMediaId ? (
                <img src={mediaContentUrl(coverMediaId)} alt="" className="dsh-pub-thumb sm" />
              ) : (
                <div className="dsh-pub-thumb sm"><IconPaperclipOutline16 size={18} /></div>
              )}
              {coverBlocked ? (
                <span className="dsh-pub-cover-note">
                  {t('form.coverDisabled', { platforms: caps.cover.blockedPlatforms.join(', ') })}
                </span>
              ) : (
                <span className="dsh-pub-cover-note">{coverMediaId ? <IconCheckOutline16 size={14} /> : '—'}</span>
              )}
            </div>
          </div>
        ) : null}
...
function TypeCard({ t, value, onPick }) {
  ...
  return (
    ...
    >
      <span className="dsh-pub-type-icon"><Icon size={20} /></span>
      <span className="dsh-pub-type-name">{t(`type.${value}`)}</span>
      <span className="dsh-pub-type-hint">{t(`type.${value}.hint`)}</span>
    </Button>
  )
}
...
function Hint({ text, tone = 'muted' }) {
  return <div className={tone === 'warn' ? 'dsh-pub-hint warn' : 'dsh-pub-hint'}>{text}</div>
}
