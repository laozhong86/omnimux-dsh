...
/**
 * Mount the `/dsh-publish` prefix on the official webServer seat.
  ...
  const dispose = webServer.register({
    ...
    async handler(req, res) {
      try {
        ...
        const url = new URL(req.url || '/dsh-publish/state', 'http://127.0.0.1')
        ...
        if (method === 'GET' && path === '/dsh-publish/records/detail') {
          const id = url.searchParams.get('id') || ''
          const result = await dispatcher.getRecord({ record_id: id, refresh: false })
          sendJson(res, 200, result)
          return
        }
        if (method === 'GET' && path === '/dsh-publish/capabilities') {
          sendJson(res, 200, dispatcher.capabilities())
          return
        }
        if (method === 'GET' && path === '/dsh-publish/media/content') {
        ...
        if (method === 'POST' && path === '/dsh-publish/media') {
        ...
        if (method === 'POST' && path === '/dsh-publish/drafts') {
        ...
        if (method === 'POST' && path === '/dsh-publish/records/submit') {
          ...
          dispatcher.submitDispatch({ record_id: recordId }).catch((error) => {
            ...
            try {
              ...
              console.error('[dsh-publish] submit runner failed without a ledger row:', message)
            }
          })
          sendJson(res, 200, { started: true, record_id: recordId })
          return
        }
        if (method === 'POST' && path === '/dsh-publish/records/refresh') {
        ...
        if (method === 'POST' && path === '/dsh-publish/tasks/retry') {
