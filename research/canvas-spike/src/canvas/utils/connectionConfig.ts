 ...
 *
 * 改动（均为裁剪，不改语义）：
 * - 裁掉 groupNode / videoComposition / 结果节点的分支（spike 只搬 material 节点）
 * - 裁掉 mediaConnectionConfig 依赖（MediaNode 未搬）
 * - getNodeOutputInfo 中 hasMaterialMediaOutput 简化为 mediaUrl 判断
 *   （原实现还查 mediaSource/mediaFiles/mediaSources/cloudFileId）
 * - isNodeConnectionValid / getNodeInputRequirements / 类型矩阵原样保留
 */
...
function getNodeInputRequirements(node: { type?: string; data?: Record<string, unknown> }): NodeInputRequirements {
  ...
  if (nodeType === 'material') {
    ...
    return { nodeType, selectedTool, acceptedTypes: Array.from(acceptedTypesSet) };
  }
...
/** 根据上游素材类型推断下游节点的默认工具（原样保留） */
export function inferDefaultTool(
  sourceMaterialType: MaterialType,
  targetMaterialType: MaterialType,
): MaterialTool | null {
  const options = MATERIAL_OUTPUT_OPTIONS[sourceMaterialType];
  if (!options) return null;
  const match = options.find((opt) => opt.targetMaterialType === targetMaterialType);
  return (match?.targetTool as MaterialTool) ?? null;
}
