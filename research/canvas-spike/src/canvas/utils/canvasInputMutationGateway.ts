 ...
 *
 * 画布输入变更网关：所有 nodes/edges 的结构性变更（连线、删线、
 * 节点数据补丁）必须经 planCanvasInputMutation 统一校验后落地。
 *
 * 改动：
 * - 裁掉模型资格校验（resolveCanvasModelEligibility）：
 *   Gxgen 原实现在 mutation 后会重算受影响节点的模型可用性并
 *   自动回写 sceneOptionId/modelInputProfileId。spike 无预设服务，
 *   空模型列表时原逻辑本身就跳过，故直接省略该循环。
 * - 结构校验、nodePatch、去重、级联删除逻辑原样保留。
 */
